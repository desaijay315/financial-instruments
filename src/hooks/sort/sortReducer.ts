    ...
    switch (action.type) {
        case TOGGLE_SORT: {
            ...
            if (column === ASSET_CLASS) {
                newSortConfig.assetClass = sortDirectionToggle(
                    state.sortConfig.assetClass
                )
            } else {
                newSortConfig[column] =
                    state.sortConfig[column] === NONE
                        ? DESC
                        : sortDirectionToggle(state.sortConfig[column])
            }

            sortedData.sort((a, b) =>
                sortFunctions[column](a, b, newSortConfig[column])
            )
